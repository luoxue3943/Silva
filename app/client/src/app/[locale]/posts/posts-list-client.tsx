...
export default function PostsListClient({ category }: PostsListClientProps) {
  // 将当前分类绑定进分页请求 / Bind the active category into paginated requests
  ...
  // 列表底部进入视口时加载下一页 / Load the next page when the list sentinel enters the viewport
  ...
  return (
    <section className={Modules["posts-section"]}>
      <div className={Modules["posts-list"]}>
        {items.map((post) => {
          // 每张卡片只格式化一次展示日期 / Format the display date once per card
