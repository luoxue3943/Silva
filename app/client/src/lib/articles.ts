 ...
 * Reads Markdown from the project article directory and provides fallback content when missing.
 */

const ARTICLE_ROOT = path.join(process.cwd(), "article");

/**
 * 读取指定文章的 Markdown 内容 / Reads Markdown content for the given post.
 */
export async function readArticleMarkdown(postId: number) {
  ...
  try {
    ...
    // 本地文章缺失时仍返回可渲染内容 / Keeps rendering possible when the local article file is missing.
